...
def naiveSoftmaxLossAndGradient(
    ...
    error = yhat # (y-yhat) # shape (num words in vocab, )
    error[outsideWordIdx] -= 1
    gradCenterVec = np.dot(error, outsideVectors) #shape (word vector length, )
    gradOutsideVecs = np.outer(error, centerWordVec) #shape (num words in vocab, word vector length)
...
def negSamplingLossAndGradient(
    ...
    score[1:] *= -1
    score = sigmoid(score)
    loss = -np.log(score).sum()
    score = 1 - score
    score[0] *= -1
    gradCenterVec = np.dot(outsideVectors[indices].T, score)
    gradOutsideVecs = np.zeros(outsideVectors.shape)
    _gradOutsideVecs = np.outer(score, centerWordVec)
    np.add.at(gradOutsideVecs, indices, _gradOutsideVecs)
    ### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for word in outsideWords:
        current_loss, current_gradCenterVec, current_gradOutsideVecs = \
            word2vecLossAndGradient(centerWordVec, word2Ind[word], 
                                    outsideVectors, dataset)
        loss += current_loss
        gradCenterVecs[centerWordIdx] += current_gradCenterVec
